/**
 * Lab-MeadowIntegration-BeaconProvider
 *
 * Lab-local ultravisor-beacon capability provider wrapping meadow-integration's
 * FileParser + TabularTransform services and a small LabWriter action that
 * posts records to a databeacon's dynamic /1.0/<Entity> endpoint.
 *
 ...
 * (`LabWriter`) that seed operations target:
 *
 ...
 *
 * Used by the quick-seed flow in Service-SeedDatasetManager: when the user
 * clicks "Seed into engine", the lab auto-provisions a meadow-integration
 * beacon (hosted by lab-beacon-host.js loading this provider) on the target
 * Ultravisor if one isn't already present.
 *
 * Lives in the lab rather than the meadow-integration module so we don't
 * have to touch meadow-integration just to supply a CapabilityProvider
 * class -- eventually meadow-integration itself can ship one and we drop
 * this shim.
 */
'use strict';
...
class LabMeadowIntegrationBeaconProvider
{
	/**
	 * pProviderConfig is the user's saved config blob (empty object is fine
	 * for this provider).  pPict is the beacon-host's pict instance; when
	 * supplied, meadow-integration's services register through its
	 * serviceManager (their native wiring) and we grab the instances off
	 * the pict.  The pict arg is optional for safety but strongly
	 * recommended -- parseContent/transform expect a real fable host.
	 */
	constructor(pProviderConfig, pPict)
	{
		...
		if (!pPict)
		{
			throw new Error('LabMeadowIntegrationBeaconProvider requires the host pict instance; update lab-beacon-host to pass it.');
		}
...
module.exports = LabMeadowIntegrationBeaconProvider;
